...
📄 [Read the full thesis (PDF)](/files/pirate-rush-vr-thesis.pdf)

## Technical Highlights

- **Runtime mesh slicing with the divergence theorem.** The sabre cuts arbitrary fruit meshes live along the blade's swing plane using EzySlice, then gives each half its own collider and rigidbody so the two pieces fall apart naturally. Scoring a clean cut on an irregular mesh is the harder part, since a bounding box tells you nothing about where the blade passed, so I compute the mesh's exact volume with the divergence theorem (one signed tetrahedron per triangle) and compare both halves against a tolerance tuned per difficulty. Code in [`Sabre.cs`](https://github.com/szejkerek/PirateRushVR/blob/main/Assets/__Scripts/Player/Weapons/Sabre.cs).
